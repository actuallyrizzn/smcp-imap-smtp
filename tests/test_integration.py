...
class TestEndToEndWorkflows:
    ...
    def test_send_and_fetch_workflow(self, mock_smtp_connect, mock_imap_connect):
        """Test sending an email and then fetching it."""
        # Mock SMTP connection
        mock_smtp_conn = Mock()
        mock_smtp_conn.send_email.return_value = {'message_id': '<test@example.com>'}
        mock_smtp_connect.return_value = mock_smtp_conn
        
        # Mock IMAP connection
        mock_imap_conn = Mock()
        mock_imap_conn.search.return_value = [1]
        mock_imap_conn.fetch_messages.return_value = [{
            'id': 1,
            'subject': 'Test Email',
            'from': {'email': 'sender@example.com'},
            'body': {'text': 'Test body'}
        }]
        mock_imap_connect.return_value = mock_imap_conn
    ...
    @patch('tools.imap.cli._auto_connect')
    def test_search_and_mark_read_workflow(self, mock_imap_connect):
    ...
    @patch('tools.imap.cli._auto_connect')
    def test_sandbox_workflow(self, mock_imap_connect):
